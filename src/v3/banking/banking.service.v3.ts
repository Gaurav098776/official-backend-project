...
import { NetbankingServiceV3 } from './netbanking.service.v3';
import { IpCheckService } from 'src/shared/ipcheck.service';
import { ICreateNewAAJourney } from './banking.interfaces';
...
export class BankingServiceV3 {
  constructor(
    ...
    private readonly kycServiceV3: KycServiceV3,
    // Services
    private readonly netbanking: NetbankingServiceV3,
  ...
  async getList(reqData) {
    ...
    const options = {
      where: { userId, accountNumber: { [Op.ne]: null } },
      order: [['id', 'DESC']],
    };

    const accNumber = await this.repository.getRowWhereData(
      attributes,
      options,
    );
  ...
  async getNewList(reqData) {
    ...
    let bankList = await this.sharedBanking.getNewBankList(reqData);
    if (bankList?.message) return bankList;
    ...
    bankList = bankList.filter((el) => {
      ...
      if (el.service.aa != -1) el.availableServices.push('Account Aggregator');
      ...
      if (el.availableServices.length === 0) {
        return false;
      } else {
        delete el.service;
        return true;
  ...
  async syncAAData(data: any) {
    try {
      ...
      if (regPanCard(data?.pan)) {
        ...
        await this.kycServiceV3.validatePan(kycData, true);
      }
