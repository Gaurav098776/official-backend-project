...
import { kKeyFactStatement } from 'src/constants/objects';
import {
  k422ErrorMessage,
  kBadRequest,
  kInternalError,
...
export class LoanServiceV3 {
  constructor(
    ...
    private readonly kycService: KycServiceV3,
  ...
  async updateEmiDate(reqData) {
    try {
      ...
      const score = await this.sharedEligiblityService.getAmountRange(
        loanId,
        {},
      );
  ...
  async acceptAmount(reqData) {
    try {
      ...
      const insuranceOptValue: any = false;
      // const insuranceOptValue: boolean = reqData.insuranceOptValue ?? true;
      ...
      if (
        ...
        const nomineeRelationship = valueInsurance.relationshipCode;
        return { needUserInfo: true, nomineeRelationship };
      }
      // KFS flow
  ...
  private async getKeyFactStatement(id) {
    try {
      ...
      const KFSDoc: any = await this.addDynamicValues(
        kKeyFactStatement,
        KFSData,
        path,
      );
  ...
  private async convertDataToObject(loanData) {
    try {
      ...
      const parsedEmiData = loanData?.netEmiData.map((item) =>
        JSON.parse(item),
      );
      let totalInterest = 0;
      parsedEmiData.map((emi) => {
      ...
      const percentageChange =
        ((totalAmount - disbursedAmount) / disbursedAmount) * 100;
      ...
      const KFSData = [];
      KFSData.push(userData?.selfieData?.image);
      KFSData.push(signingDate);
      KFSData.push(userData?.fullName);
      KFSData.push(NBFC_NAME);
      KFSData.push(loanData?.id);
      KFSData.push('₹' + loanData?.netApprovedAmount);
      KFSData.push(loanData?.approvedDuration);
      KFSData.push(loanData?.interestRate);
      KFSData.push(parsedEmiData.length);
      KFSData.push(anumm.toFixed(3));
      KFSData.push(signingDate);
      KFSData.push(
      ...
      KFSData.push(endDate);
      KFSData.push(loanData.processingFees);
      KFSData.push(loanFees);
      KFSData.push(loanData?.charges['doc_charge_per'] ?? '');
      KFSData.push(documentChargeAmount);
      KFSData.push(sgstChargeAmount);
      KFSData.push(cgstChargeAmount);
      KFSData.push('₹ 500');
      penaltyPerAnnum = anumm * 2;
      KFSData.push((+loanData?.interestRate * 2).toFixed(3));
      KFSData.push(penaltyPerAnnum.toFixed(3));
      KFSData.push(riskAssessmentPr ?? '');
      if (isStamp) KFSData.push(kRuppe + (+stampFees).toFixed(2));
      else KFSData.push(kRuppe + risk_assessment_charge.toFixed(2));
      KFSData.push('-');
      KFSData.push('-');
      KFSData.push('-');
      KFSData.push('-');
      ...
      KFSData.push('-');
      KFSData.push('-');
      KFSData.push(kHelpContact);
      KFSData.push(kSupportMail);
      KFSData.push(kCollectionEmail);
      ...
      // else premiumAmount = '-';
      // KFSData.push(premiumAmount);
      // // User has opted out for insurance
      // if (loanData.insuranceOptValue == false) KFSData.push('-');
      // else KFSData.push('Care Health');
      KFSData.push(`${LOAN_AGREEMENT_COOLINGOFF_PERIOD} Days`);
      KFSData.push(tempInsurance_fee);
      KFSData.push(loanData?.interestRate);
      const lendingPartner =
        appType === 0
          ? `<div style="font-size: 12px; color: #fff; margin-bottom: 18px">
          Lending Partner : LENDITT
      </div>`
          : '';

      KFSData.push(lendingPartner);
      KFSData.push(aprCharges);
      KFSData.push(
  ...
  private async addDynamicValues(
    staticValues: string[],
    dynamicValues: string[],
    filePath: string,
  ) {
    try {
      if (staticValues.length != dynamicValues.length) return kBadRequest;
      let content = fs.readFileSync(filePath, 'utf8');
      for (let index = 0; index < staticValues.length; index++) {
        try {
          const staticData = staticValues[index];
          const dynamicData = dynamicValues[index].toString();
          if (staticData == '##selfieImg##') {
            ...
            content = content.replace('##deg##', sty);
            await this.fileService.removeFile(imagePath);
          }

          content = this.typeService.replaceAll(
            content,
            staticData,
            dynamicData,
          );
  ...
  async acceptKFS(reqData) {
    try {
      ...
      const include = [loanInclude];
      const attributes = ['id', 'status', 'userId', 'dates'];
      const options = { include, where: { loanId } };
      const masterData = await this.masterRepo.getRowWhereData(
        attributes,
        options,
      ...
      if (
        ...
        if (!GLOBAL_FLOW.REFERENCE_IN_APP && statusData.pan == 5) {
          const panResponse: any = await this.kycService.validatePan({
            userId: reqData.userId,
          });
          if (panResponse.message) return panResponse;
          statusData.pan = panResponse.panStatus ?? statusData.pan;
        }

        return await this.getupdateRefe(
          loanId,
          statusData,
          dates,
          masterId,
      ...
      for (let index = 0; index < loanList.length; index++) {
        try {
          ...
          const obj: any = {
            ...
            netApprovedAmount: loan?.netApprovedAmount,
  ...
  async disburesmentSmsData(loanData) {
    try {
      const templeteData = await this.templeteRepo.getRowWhereData(
        ['id', 'templateId'],
        { where: { templateId: MSG91Templete.DISBUREMENT } },
      );
      if (templeteData == k500Error || !templeteData) return kInternalError;
      const userData = loanData.registeredUsers;
  ...
  async getLoanDetails(query) {
    try {
      ...
      const include = [emiInc, disbInc, eSignInc];
      const attributes = [
        'id',
        'loanFees',
        'stampFees',
    ...
    try {
      ...
      for (let index = 0; index < traLength; index++) {
        try {
          ...
          const method =
            response?.method ?? response?.items
  ...
  async acceptAmountCharges(reqData) {
    try {
      ...
      if (!amount) return kParamMissing('amount');
      if (
        amount < GLOBAL_RANGES.MIN_LOAN_AMOUNT ||
        amount > GLOBAL_RANGES.MAX_LOAN_AMOUNT
      )
      ...
      return await this.prepareAcceptScreen(calculation);
    } catch (error) {
      return kInternalError;
    }
  }
  ...
  async prepareAcceptScreen(data) {
    try {
      const result: any = {};
      ...
      const premium = insurance?.totalPremium ?? 0;
      let disburseAmount =
        amount -
        processingFees -
        stampFees -
      ...
      result['insuranceOptValue'] =
        data?.insuranceOptValue ?? GlobalServices.INSURANCE_OPT_VALUE ?? false;
      if (
        GlobalServices.INSURANCE_SERVICE &&
        GlobalServices.INSURANCE_SERVICE != 'NONE'
      ) {
      ...
      if (result['isInsurance']) disburseAmount -= premium;
      ...
      const penaltyRate = (interest * 2).toFixed(3);
      result[
        'latePaymentTag'
