...
export class KycServiceV4 {
  ...
  async validatemAadhaar(reqData) {
    try {
      ...
      const customerRefId = aadhaarNumber
  ...
  private async isAadhaarExists(userId, aadhaarNumber) {
    try {
      ...
      const queryString = `SELECT "aadhaarNo" FROM public."KYCEntities" WHERE "aadhaarNo" = '${aadhaarNo}' AND "userId" != '${userId}' LIMIT 1;`;
  ...
  async aadhaarOtpRequest(data) {
    ...
    if (existingData.message) return existingData;
    const userAttr = ['id', 'typeOfDevice'];
    ...
    if (data.typeOfDevice === '2') aadhaar_service = 'ZOOP';
    // Flow -> Digilocker
    let response: any;
    if (
