...
import { EmploymentServiceV4 } from './employment.service.v4';
import { IPConfig } from 'src/utils/custom.decorators';

@Controller('employment')
export class EmploymentControllerV4 {
  constructor(
    private readonly service: EmploymentServiceV4,
  ...
  @Post('submitDetails')
  async funSubmitDetails(@Body() body, @Res() res, @Headers() headers) {
    try {
  ...
  @Get('expressSubmitDetails')
  async funExpressSubmitDetails(@Query() query, @Res() res) {
    try {
      let data: any = await this.service.expressSubmitDetails(query);
      if (data?.message) return res.json(data);
      if (data.needUserInfo) {
        data = await this.userService.routeDetails({ id: query.userId });
        if (data?.message) return res.send(data);
  ...
  @Post('reApplySubmitDetails')
  async funReApplySubmitDetails(@Body() body, @Res() res) {
    try {
      let data: any = await this.service.reApplySubmitDetails(body);
      if (data?.message) return res.send(data);
      if (data.needUserInfo) {
        data = await this.userService.routeDetails({ id: body.userId });
        if (data?.message) return res.send(data);
