...
export class EmploymentServiceV4 {
  ...
  private async doubleCheckMinSalary({
    ...
    if (userEnteredSalary && !isNaN(userEnteredSalary)) {
      if (userEnteredSalary < GLOBAL_RANGES.MIN_SALARY_AMOUNT) {
        if (!GLOBAL_FLOW.SALARY_COOLOFF_BEFORE_STATEMENT && userData?.appType == 0 && userEnteredSalary >= GLOBAL_RANGES.MIN_EMPLOYMENT_SALARY) {
  ...
  private async handleReApplyFlow(
    ...
    try {
      ...
      const userId = reqData.userId;
      const userAppData = await this.userRepo.getRowWhereData(['appType'], {
        where: { id: userId },
      });
      const appType = userAppData?.appType ?? 0;
      let completedLoan = 0;
      const where = { userId };
      ...
      if (activeLoan) return k422ErrorMessage('Loan is already Active!');
      ...
      const creationData: any = {
        ...
        loanAcceptStatus: -1,
        kfsStatus: -1,
        initialTypeOfDevice: userData.typeOfDevice,
        ...(updateLoanData ?? {}),
      };
      if (companyName && companyId) creationData.companyId = companyId;
  ...
  async getFieldsForSameCompany(query) {
    try {
      ...
      kLspEmploymentFields.forEach((data: any) => {
      ...
      if (isNew == 'true' && appType == '0') return kLspEmploymentFields;
      if (isNew == 'true') return kLspEmploymentFields;
  ...
  async expressSubmitDetails(reqData) {
    ...
    if (newCompany === 'YES') {
      ...
      data = { purposeList, communicationLanguage };
    } else if (loanPurpose && !salary) {
      body = {
        userId: userId,
        communicationLanguage: communicationLanguage,
        purposeId: loanPurpose,
      };
      response = await this.userService.submitBasicDetails(body);
      if (response.message) return response;
      data = await this.getCompanyData(userId);
      ...
      data = await this.sharedBankingService.getBankData(userId);
    } else if (loanPurpose && salary && oldBank === 'YES') {
      data = {
        nextRoute: kTransferFundsUiV2,
      };
    } else if (loanPurpose && salary && oldBank === 'NO') {
      data = {
        nextRoute: kNetBankingRoute,
      };
    }
    if (loanId) {
  ...
  async reApplySubmitDetails(reqData) {
    ...
    const netPaySalary =
      +this.typeService.checkAndRemoveComma(salary ?? '') ?? '';
